...
import 'assessmentpage_isItHardForYouuToTakeCare.dart';

class AssessmentPage16 extends StatefulWidget {
  const AssessmentPage16({Key? key}) : super(key: key);

  @override
  _AssessmentPage1State createState() => _AssessmentPage1State();
}

class _AssessmentPage1State extends State<AssessmentPage16> {
  AssessmentPagesSerivces controllerAssessmentPageServices = Get.find();
  int _selectedIndex = -1;
  List<String> answerList = ["Never", "Rarely", "Sometimes", "Quite Often", "Frequently", "Nearly Always", "Unknown / NA"];
          ...
          children: [
            const SizedBox(
              child: AssessmentQuestions(
                text: "Do you fell strained (i.e. stressed, tense or anxious) when you are around the person you are caring for?",
              ),
            ),
            ...
            Row(
              children: [
                Container(color: kConstantPurple, height: 5.h, width: 255.w),
                Container(color: formLandingColor4, height: 5.h, width: 120.w),
            ...
            ExitBackAndNextButtonsList(
                next: () {
                  if (_selectedIndex != -1) {
                    controllerAssessmentPageServices.submitAns(14, _selectedIndex);
                  }
                  Get.to(const AssessmentPage17());
                },
